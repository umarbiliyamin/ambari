...
class MZ10NIFIServiceAdvisor(service_advisor.ServiceAdvisor):
    def __init__(self, *args, **kwargs):
        self.as_super = super(MZ10NIFIServiceAdvisor, self)
    ...
    def getServiceComponentLayoutValidations(self, services, hosts):
        items = super(MZ10NIFIServiceAdvisor, self).getServiceComponentLayoutValidations(services, hosts)
        return items
    ...
    def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
        ...
        if "ranger-env" in services["configurations"] and "ranger-nifi-plugin-properties" in services[
            ...
            rangerEnvNiFiPluginProperty = services["configurations"]["ranger-env"]["properties"][
            ...
            if rangerEnvNiFiPluginProperty == 'Yes' and \
                        ...
                        "properties"] and \
                            "nifi.node.ssl.isenabled" in services["configurations"]["nifi-ambari-ssl-config"][
                        "properties"]:
                nifiAmbariSSLConfig = 'SSL' if services["configurations"]["nifi-ambari-ssl-config"]["properties"][
        ...
        for item in ranger_audit_dict:
            if item['filename'] in services['configurations'] and item['configname'] in \
                ...
                if item['filename'] in configurations and item['configname'] in configurations[item['filename']][
                    'properties']:
    ...
    def validateConfigurationsForSite(self, configurations, recommendedDefaults, services, hosts, siteName, method):
        ...
        if properties:
            if siteName == 'nifi-ambari-ssl-config' or siteName == 'nifi-ambari-config':
                ...
                return super(MZ10NIFIServiceAdvisor, self).validateConfigurationsForSite(configurations,
    ...
    def validateNiFiSslProperties(self, properties, recommendedDefaults, configurations, services, hosts):
        ...
        if ssl_enabled and not properties['nifi.initial.admin.identity']:
            validationItems.append({"config-name": 'nifi.initial.admin.identity', 'item': self.getWarnItem(
        ...
        if (self.__find_ca(services)):
            if not properties['nifi.toolkit.tls.token']:
                validationItems.append({"config-name": 'nifi.toolkit.tls.token', 'item': self.getErrorItem(
                    'If NiFi Certificate Authority is used, nifi.toolkit.tls.token must be set')})
            if not ssl_enabled:
                validationItems.append({"config-name": 'nifi.node.ssl.isenabled', 'item': self.getWarnItem(
                    'For NiFi Certificate Authority to be useful, ssl should be enabled')})
        else:
            if properties['nifi.toolkit.tls.token']:
                validationItems.append({"config-name": 'nifi.toolkit.tls.token', 'item': self.getWarnItem(
            ...
            if ssl_enabled:
                if not properties['nifi.security.keystorePasswd']:
                    validationItems.append({"config-name": 'nifi.security.keystorePasswd', 'item': self.getErrorItem(
                ...
                if not properties['nifi.security.keyPasswd']:
                    validationItems.append({"config-name": 'nifi.security.keyPasswd', 'item': self.getErrorItem(
                ...
                if not properties['nifi.security.truststorePasswd']:
                    validationItems.append({"config-name": 'nifi.security.truststorePasswd', 'item': self.getErrorItem(
                ...
                if not properties['nifi.security.keystoreType']:
                    validationItems.append({"config-name": 'nifi.security.keystoreType', 'item': self.getErrorItem(
                ...
                if not properties['nifi.security.truststoreType']:
                    validationItems.append({"config-name": 'nifi.security.truststoreType', 'item': self.getErrorItem(
    ...
    def validateNiFiAmbariConfigurations(self, properties, recommendedDefaults, configurations, services, hosts):
        ...
        if len(properties['nifi.sensitive.props.key']) < 10:
            validationItems.append({"config-name": 'nifi.sensitive.props.key', 'item': self.getWarnItem(
