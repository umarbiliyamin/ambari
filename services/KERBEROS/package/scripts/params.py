...
if config is not None:
    ...
    if configurations is not None:
        ...
        if cluster_env is not None:
            ...
            smoke_user = get_property_value(cluster_env, 'smokeuser',
                                            smoke_user, True, smoke_user)
        ...
        if kerberos_env is not None:
            manage_identities = get_property_value(
                kerberos_env, "manage_identities", "true", True, "true")
            encryption_types = get_property_value(
                kerberos_env, "encryption_types", None, True, None)
            realm = get_property_value(kerberos_env, "realm", None, True, None)
            kdc_hosts = get_property_value(kerberos_env, 'kdc_hosts',
                                           kdc_hosts)
            admin_server_host = get_property_value(
                kerberos_env, 'admin_server_host', admin_server_host)
        ...
        if krb5_conf_data is not None:
            ...
            admin_principal = get_property_value(
                krb5_conf_data, 'admin_principal', admin_principal, True, None)
            admin_password = get_property_value(
                krb5_conf_data, 'admin_password', admin_password, True, None)
            admin_keytab = get_property_value(krb5_conf_data, 'admin_keytab',
                                              admin_keytab, True, None)

            test_principal = get_property_value(
                krb5_conf_data, 'test_principal', test_principal, True, None)
        ...
        if kdc_conf_data is not None:
            kdcdefaults_kdc_ports = get_property_value(
                kdc_conf_data, 'kdcdefaults_kdc_ports', kdcdefaults_kdc_ports)
            kdcdefaults_kdc_tcp_ports = get_property_value(
                kdc_conf_data, 'kdcdefaults_kdc_tcp_ports',
    ...
    if command_params is not None:
        ...
        if manage_identities:
            smoke_test_principal = get_property_value(
                command_params, 'principal_name', smoke_test_principal)
            smoke_test_keytab_file = get_property_value(
