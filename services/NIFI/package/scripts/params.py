...
nifi_dir = install_dir = config['configurations']['nifi-env']['install_dir']
download_url = config['configurations']['nifi-env']['download_url']
filename = download_url.split('/')[-1]
version_dir = filename.replace('.tar.gz', '').replace('.tgz', '')

install_dir_toolkit = config['configurations']['nifi-env']['install_dir_toolkit']
download_url_toolkit = config['configurations']['nifi-env']['download_url']
filename_toolkit = download_url_toolkit.split('/')[-1]
version_dir_toolkit = filename_toolkit.replace('.tar.gz', '').replace('.tgz', '')
nifi_config_toolkit_dir = '/etc/nifi/toolkit'
...
nifi_provenance_repo_dir_default = config['configurations']['nifi-ambari-config'][
...
nifi_security_encrypt_configuration_password = config['configurations']['nifi-ambari-config'][
...
if 'nifi.content.repository.dir.default' in config['configurations']['nifi-ambari-config']:
    nifi_content_repo_dir_default = config['configurations']['nifi-ambari-config'][
        'nifi.content.repository.dir.default']

nifi_content_repo_dirs = [v for k, v in config['configurations']['nifi-properties'].items() if
                          k.startswith('nifi.content.repository.dir')]
...
# kerberos params
nifi_kerberos_authentication_expiration = config['configurations']['nifi-properties'][
...
if security_enabled:
    ...
    nifi_properties['nifi.kerberos.service.principal'] = nifi_properties['nifi.kerberos.service.principal'].replace(
        '_HOST', _hostname_lowercase)
    nifi_properties['nifi.kerberos.spnego.principal'] = nifi_properties['nifi.kerberos.spnego.principal'].replace(
        '_HOST', _hostname_lowercase)
...
if has_ranger_admin:
    ...
    driver_curl_target = format(
        "{stack_root}/current/nifi/ext/{jdbc_jar_name}") if stack_supports_ranger_audit_db else None
    previous_jdbc_jar = format(
        "{stack_root}/current/nifi/ext/{previous_jdbc_jar_name}") if stack_supports_ranger_audit_db else None
    sql_connector_jar = ''

    ssl_keystore_password = unicode(config['configurations']['ranger-nifi-policymgr-ssl'][
                                        'xasecure.policymgr.clientssl.keystore.password']) if xml_configurations_supported else None
    ssl_truststore_password = unicode(config['configurations']['ranger-nifi-policymgr-ssl'][
    ...
    if nifi_authentication == 'SSL':
        ...
        nifi_ranger_plugin_config = {
            ...
            'nifi.ssl.keystorePassword': config['configurations']['ranger-nifi-plugin-properties'][
                'nifi.ssl.keystorePassword'],
            'nifi.ssl.truststore': config['configurations']['ranger-nifi-plugin-properties']['nifi.ssl.truststore'],
            'nifi.ssl.truststoreType': config['configurations']['ranger-nifi-plugin-properties'][
                'nifi.ssl.truststoreType'],
            'nifi.ssl.truststorePassword': config['configurations']['ranger-nifi-plugin-properties'][
    ...
    if stack_supports_ranger_kerberos and security_enabled:
        ...
        ranger_nifi_principal = config['configurations']['nifi-properties']['nifi.kerberos.service.principal'].replace(
            '_HOST', _hostname_lowercase)
...
hadoop_bin_dir = '/opt/hadoop/bin' if has_namenode else None
