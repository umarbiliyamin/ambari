...
install_dir = config['configurations']['solr-env']['install_dir']
download_url = config['configurations']['solr-env']['download_url']
...
solr_cloudmode = 'true'
solr_dir = install_dir
solr_client_dir = install_dir
...
solr_datadir = default('/configurations/solr-env/solr_datadir', '/data/solr')
...
if has_ranger_admin and is_supported_solr_ranger:
    ...
    ranger_plugin_config = {
        'username': default('/configurations/ranger-env/ranger_admin_username', 'ranger_admin'),
        'password': unicode(default('/configurations/ranger-env/ranger_admin_password', 'EXAMPLE.COM')),
        'solr.url': config['configurations']['ranger-solr-plugin-properties']['solr.url'],
        'commonNameForCertificate': config['configurations']['ranger-solr-plugin-properties'][
    ...
    ssl_keystore_password = unicode(config['configurations']['ranger-solr-policymgr-ssl'][
                                        'xasecure.policymgr.clientssl.keystore.password']) if xml_configurations_supported else None
    ssl_truststore_password = unicode(config['configurations']['ranger-solr-policymgr-ssl'][
    ...
    setup_ranger_env_sh_source = format(
        '{stack_root}/ranger-solr-plugin/install/conf.templates/enable/solr-ranger-env.sh')
...
if 'ranger-env' in config['configurations']:
    stack_root = '/opt'
    ranger_home = format('{stack_root}/ranger-admin')
