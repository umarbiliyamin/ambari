...
def install_kylin():
    import params
    if not os.path.exists('/opt/' + params.version_dir):
        ...
        Execute('chown -R %s:%s /opt/%s' %
                (params.kylin_user, params.kylin_group, params.version_dir))
...
class Job(Script):
    ...
    def chown_kylin_pid_dir(self, env):
        ...
        Execute(
            ("chown", "-R",
             format("{kylin_user}") + ":" + format("{kylin_group}"),
             params.kylin_pid_dir),
            sudo=True)
    ...
    def start(self, env, upgrade_type=None):
        ...
        Execute(
            ' source ' + params.kylin_dir + '/bin/kylin-env.sh ;' +
