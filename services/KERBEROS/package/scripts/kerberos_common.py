...
class KerberosScript(Script):
    KRB5_REALM_PROPERTIES = [
        'kdc', 'admin_server', 'default_domain', 'master_kdc'
    ]

    KRB5_SECTION_NAMES = [
        'libdefaults', 'logging', 'realms', 'domain_realm', 'capaths',
        'ca_paths', 'appdefaults', 'plugins'
    ]
        ...
        if section_name is not None:
            ...
            if realms_data is not None:
                for realm, realm_data in realms_data.iteritems():
                    KerberosScript._write_conf_realm(output_file, realm,
                                                     realm_data)
                    output_file.write('\n')
    ...
    def write_krb5_conf():
        ...
        Directory(
            params.krb5_conf_dir,
            owner='root',
            create_parents=True,
            group='root',
            mode=0755)

        if (params.krb5_conf_template is None
            ) or not params.krb5_conf_template.strip():
            content = Template('krb5_conf.j2')
        else:
            content = InlineTemplate(params.krb5_conf_template)

        File(
            params.krb5_conf_path,
            content=content,
            owner='root',
            group='root',
            mode=0644)
    ...
    def principal_exists(identity, auth_identity=None):
        ...
        if identity is not None:
            ...
            if (principal is not None) and (len(principal) > 0):
                try:
                    result_code, output = KerberosScript.invoke_kadmin(
                        'getprinc %s' % principal, auth_identity)
                    exists = (output is not None) and (
                        ("Principal: %s" % principal) in output)
                except:
                    raise Fail("Failed to determine if principal exists: %s" %
                               principal)
        ...
        if principal is not None:
            ...
            # If no keytab data is available and a password was supplied, simply use it.
                ...
                process = subprocess.Popen(
                    [kinit_path_local, principal], stdin=subprocess.PIPE)
    ...
    def write_keytab_file(self):
        ...
        if params.kerberos_command_params is not None:
            for item in params.kerberos_command_params:
                ...
                if (keytab_content_base64 is not None) and (
                        len(keytab_content_base64) > 0):
                    keytab_file_path = get_property_value(item,
                                                          'keytab_file_path')
                    if (keytab_file_path is not None) and (
                            len(keytab_file_path) > 0):
                        ...
                        owner = get_property_value(item,
                                                   'keytab_file_owner_name')
                        if not owner:
                            owner = getpass.getuser()
                        owner_access = get_property_value(
                            item, 'keytab_file_owner_access')
                        group = get_property_value(item,
                                                   'keytab_file_group_name')
                        group_access = get_property_value(
                            item, 'keytab_file_group_access')
                        mode = 0
